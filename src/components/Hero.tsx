...
const CharacterScrollReveal = ({
  ...
  const currentFrameIndex = getSequenceFrameIndex(progressRef.current)
  const currentSheetIndex = Math.floor(currentFrameIndex / FRAMES_PER_SHEET)
  const isCurrentSheetLoaded = !!sheetsRef.current[currentSheetIndex]

  const loadingProgress = Math.round((loadedSheets / SHEET_COUNT) * 100)
  const isLoaded = loadedSheets >= 1 && isCurrentSheetLoaded
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`absolute inset-0 z-30 flex flex-col items-center justify-center bg-[#050505] px-6 transition-opacity duration-1000 ${isLoaded ? 'pointer-events-none opacity-0' : 'opacity-100'
            }`}
        >
